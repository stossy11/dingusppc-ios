...
void Display::update(std::function<void(uint8_t *dst_buf, int dst_pitch)> convert_fb_cb,
    ...
    if (impl->resizing) {
        printf("Update skipped due to resizing.\n");
        return;
    }
    ...
    printf("Locking texture for update...\n");
    ...
    if (draw_hw_cursor) {
        ...
        printf("Drawing hardware cursor at (%d, %d)...\n", cursor_x, cursor_y);
        SDL_RenderCopy(impl->renderer, impl->cursor_texture, NULL, &impl->cursor_rect);
    }

    printf("Presenting the rendered frame...\n");
    SDL_RenderPresent(impl->renderer);
}
