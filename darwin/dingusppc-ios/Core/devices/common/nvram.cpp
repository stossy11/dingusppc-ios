...
void NVram::init() {
    ...
    // Construct the full path to the NVRAM file inside Documents directory
    std::string fullPath = documentsPath + "/" + this->file_name;
    ...
    if (f.fail() || !f.read(sig, sizeof(NVRAM_FILE_ID)) ||
        ...
        LOG_F(WARNING, "Could not restore NVRAM content from the file \"%s\".", fullPath.c_str());
...
void NVram::save() {
    ...
    ofstream f(fullPath, ios::out | ios::binary);
